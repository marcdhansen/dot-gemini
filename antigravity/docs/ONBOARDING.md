# 🚀 Agent Onboarding & Universal Agent Protocol
...
2. **Run Bootstrap Script**: Execute `./scripts/agent-init.sh` to automatically verify your toolchain and get up to speed on the session state.

## 1. Anchor to Global Memory

Identify global configuration directories.
> "Read the Global Agent Rules in `~/.gemini/GEMINI.md` and follow the Universal Agent Protocol (SOP). Use the Global Index at `~/.agent/docs/GLOBAL_INDEX.md` as your primary navigation map."
...
5. **Conflict Check**: Calls Orchestrator Initialization to ensure no one else is on your task.
...
- **Cleanup is mandatory**: Always run `./scripts/agent-end.sh` (included in Finalization).
...
```bash
./scripts/agent-init.sh  # Original toolchain + Initialization only
```

## 5. Connect to Project Brain

Read the following files to understand the session status:
...
- **Comprehensive Index**: `docs/sop/global-configs/GLOBAL_INDEX.md` (symbolic link to `~/.agent/docs/GLOBAL_INDEX.md`)
...
## 7. Standard Session Loop (Finalization)

You MUST execute the **Finalization** procedure before ending your session:
...
### Automem: Quick Setup (Enhanced Bootstrap)
...
### Langfuse: Quick Setup (Enhanced Bootstrap)
...
### Langfuse: Manual Setup (Fallback)
