...
name: code-search
description: >
  Finds code: where functions and classes are implemented, who calls them,
  all references to a module. Use when asking where something is
  implemented, searching for a function or class in the codebase, finding
  all files that import a module, or exploring an unfamiliar codebase.
  Do NOT use for planning features or assessing change impact (use
  planning). Read-only — does not modify code.
compatibility: >
  Requires Bash with ripgrep (rg) or grep available. No scripts to run;
  instructions guide tool usage directly.
metadata:
  author: Workshop Team
  version: "1.0.0"
  category: code-quality
  tags: [navigation, codebase, search, dependencies, architecture]
