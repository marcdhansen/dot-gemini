...
name: sop-checker
description: >
  SOP compliance checker: validates that agents correctly completed all
  required phase steps and quality gates. Use when verifying that
  initialization or finalization was done properly, running protocol
  compliance checks, or confirming phase gates were followed.
  Do NOT confuse with session-briefing (orientation) or finalization
  (doing the work) — orchestrator checks that they were done correctly.
compatibility: >
  Requires Python 3.x, bd CLI, git. Scripts live in
  ~/.gemini/antigravity/skills/sop-checker/scripts/.
  Falls back to manual checklist validation if Python is unavailable.
metadata:
  author: Workshop Team
  version: "1.0.0"
  category: orchestration
  tags: [orchestration, sop, compliance, quality-gates, session-management]
